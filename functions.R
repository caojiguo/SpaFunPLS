...
# *** basic functions
# *** functions for the SpaFunPLS method
# *** functions for the FunPLS method
...
fit.FunPLS = function(y, xind, x, domain, M, d, K, Mobs,mod.select, ncv=NULL)
{
  ...
  for(iterM in 1:length(M))
  {
    ...
    for(iterk in Kind)
    {
      RSS     = fitpls$RSS[iterk]
      Dof     = fitpls$DoF[iterk]
      sigma   = fitpls$sigmahat[iterk]
...
#- Tune the SpaFunPLS fit using BIC
